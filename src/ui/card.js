...
export function doVote(pid, t, e) {
    e.stopPropagation();
    const el = document.getElementById('pc-' + pid);
    if (el) {
        el.classList.add('blip');
        setTimeout(() => el.classList.remove('blip'), 200);
    }
    castVote(pid, t);
}
export function toggleComments(pid) {
    openComments[pid] = !openComments[pid];
    refresh();
}
export function toggleForm(pid) {
    openForms[pid] = !openForms[pid];
    if (openForms[pid])
        openComments[pid] = true;
    refresh();
    if (openForms[pid]) {
        setTimeout(() => { const el = document.getElementById('ctxt-' + pid); if (el)
            el.focus(); }, 50);
    }
}
export function doComment(pid) {
    const el = document.getElementById('ctxt-' + pid);
    if (!el || !el.value.trim())
        return;
...
export function fbCopy(txt) {
    ...
    try {
        ...
        showToast('Copied! Paste into Claude.ai 🤖', '#a78bfa');
    }
    catch (err) {
        showToast('Could not copy — try again', '#ef4444');
    }
