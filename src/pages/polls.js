...
export function rPolls() {
    const q = (document.getElementById('srch')?.value || '').toLowerCase().trim();
    const party = (document.getElementById('sel-party')?.value || '').trim();
    const region = (document.getElementById('sel-region')?.value || '').trim();
    const sort = (document.getElementById('sel-sort')?.value || 'trending');
    const c = getC(), uv = getUV();
    let list = [...POLS];
    // Type filter
    if (AF === 'National')
        list = list.filter(p => p.type === 'National');
    else if (AF === 'Governor')
        list = list.filter(p => p.type === 'Governor');
    else if (AF === 'Senator')
        list = list.filter(p => p.type === 'Senator');
    // Party filter
    if (party)
    ...
    // Search
    if (q)
        list = list.filter(p => p.name.toLowerCase().includes(q) || p.party.toLowerCase().includes(q) ||
    ...
    list.sort((a, b) => {
        ...
        if (sort === 'opposed') {
            const { op: oa } = pct(ca.s, ca.o), { op: ob } = pct(cb.s, cb.o);
            return ob - oa;
        }
        if (sort === 'polarising') {
            const { sp: sa } = pct(ca.s, ca.o), { sp: sb } = pct(cb.s, cb.o);
    ...
    if (cnt)
        cnt.innerHTML = `<span>${list.length}</span> politician${list.length !== 1 ? 's' : ''} shown`;
    if (g) {
    ...
    if (yvwrap && yvpills) {
        if (votes.length > 0) {
            ...
            if (yvcnt)
                yvcnt.textContent = `— ${votes.length} vote${votes.length !== 1 ? 's' : ''}`;
    ...
    if (sbTotal)
        sbTotal.textContent = fmt(total);
    const sbPols = document.getElementById('sb-pols');
    if (sbPols)
        sbPols.textContent = String(POLS.length);
    const sbCmts = document.getElementById('sb-cmts');
    if (sbCmts)
        sbCmts.textContent = String(totalComments());
    const sbYours = document.getElementById('sb-yours');
    if (sbYours)
        sbYours.textContent = String(votes.length);
    // Sidebar top 5 by support %
    const top5 = [...POLS].map(p => {
        const cv = c[p.id] || { s: 0, o: 0 };
