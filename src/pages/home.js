...
export function rHome() {
    ...
    if (n)
        n.innerHTML = `<em>${total === 0 ? '0' : fmt(total)}</em>`;
    ...
    const stPols = document.getElementById('st-pols');
    if (stPols)
        stPols.textContent = String(POLS.length);
    const stYours = document.getElementById('st-yours');
    if (stYours)
        stYours.textContent = String(yvotes);
    const stCmts = document.getElementById('st-cmts');
    if (stCmts)
        stCmts.textContent = String(cmtsTotal);
    ...
    if (gt)
        gt.innerHTML = byVol.slice(0, 3).map(card).join('');
    ...
    if (gp)
        gp.innerHTML = polar.slice(0, 3).map(card).join('');
}
