...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const fresh =
        cached?.computed_at &&
        Date.now() - new Date(cached.computed_at).getTime() < INSIGHTS_TTL_MS &&
        commentCount - (cached.comment_count_at_compute ?? 0) < INSIGHTS_NEW_COMMENT_TRIGGER;
    ...
    const prompt = `These are anonymous public comments about ${pol.name} (${pol.role}, ${pol.party}) on a Nigerian political sentiment platform. Each is prefixed with the commenter's declared stance.

Return strict JSON with:
- temperature: 0 to 100 measuring the emotional intensity of this debate, not its direction. 0 is calm and factual, 100 is furious on all sides.
- dominantEmotions: 2 to 4 lowercase single words naming the dominant emotions, for example frustration, hope, anger, pride.
- summary: one or two neutral sentences describing the character of the debate. Do not take a side.
- digestSupport: what supporters mainly argue, neutral tone, maximum 80 words. If supporters are absent, say so plainly.
- digestOppose: what critics mainly argue, neutral tone, maximum 80 words. If critics are absent, say so plainly.

Comments:
${lines.join('\n')}`;

    try {
        const out = await generateJson<InsightOut>(prompt, SCHEMA, 15000);
        const row = {
